...
class BaseStage(ABC):
    ...
    def _validate_connectivity(self, obstacle_mask, sources_pos):
        # type: (torch.Tensor, List[Tuple[int, int]]) -> bool
        ...
        stack = sources_pos[:]  # copy as we will pop it [(source_i, source_j)]
        for (source_i, source_j) in sources_pos:
    ...
    def generate_obstacles(self, size, sources_pos):
        # type: (int, List[Tuple[int, int]]) -> torch.Tensor
        ...
        if not self._validate_connectivity(obstacle_mask, sources_pos):
    ...
    def generate_simulation_temporal_sequence(self, n_steps, size):
        ...
        for _ in range(n_sources):
            ...
            sources_pos.append((i0, j0))
        
        # Génération d'obstacles selon l'étape
        obstacle_mask = self.generate_obstacles(size, sources_pos)
